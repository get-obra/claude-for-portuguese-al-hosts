...
1. **The guest's image lands on the host's hardware**, not on a platform's servers, not on a vendor's servers. The guest can be told this and the architecture demonstrates it.
2. **The data is processed locally** by Claude running on the host's machine. The reasoning core never sees a centralized database of identity images. The host's runtime never has more than one guest's image at a time, and discards it after the regulatory submission is acknowledged.
...
The platform is called **Obra**. It is built on **Claude**. It deploys locally on the host's hardware. It runs guest-identity collection through a structurally trustable channel the host controls. It produces a hash-chained audit trail the host owns. It pairs every regulator-touching action with a human verification step the host signs off on, before the action runs. It is being released as a managed service, with the vertical-specific skills released open-source under Apache 2.0.
