...
Portuguese law requires her to register the guest's identity with the authorities within twenty-four hours of arrival, verified against a passport or equivalent document. The law has been on the books for more than a decade. The penalty for missing a submission is substantial. Most hosts she knows have never been audited.

A few days before the guest arrives, she sends a polite message through the booking platform's chat. *Hello, we are excited to have you as our guest. We need to ask you for some information to provide to the authorities. We are legally required to. Could you please send a photo of your passport so I can register it.* Her English is uncertain. The message arrives in the guest's app translated by an algorithm, where it reads colder than she intended.

Eight times out of ten, the guest refuses.

The reasons vary. He does not want to send a passport image through a booking platform's chat. He does not know what *SIBA* is or whether it is real. He has heard stories about identity theft and is wary on principle. He offers to send the numbers instead, typed into the chat. She accepts what she can get. She types the numbers into the SIBA portal. She does not verify them against a document she has not seen. She submits.
...
Portugal's *Decreto-Lei n.º 128/2014*, with its subsequent amendments, obliges anyone operating short-term rental accommodation to register the identity of every foreign guest with the relevant authority within a short window of their arrival. The mechanism is a web portal where the host enters the guest's name, document number, date of birth, nationality, country of issue, and the dates of the stay. The portal stores nothing about the host's verification process. It accepts whatever the host types.
...
There is a second lever here, beyond the architecture. The passport ask fails partly because of the channel and partly because of the ask itself. Because Obra drafts the request, it can write it the way a careful, fluent host would: in the guest's own language, in the host's voice, explaining plainly why the law requires it and what becomes of the image afterward. And because the same reasoning core reads the guest's replies, it can recognize the specific worry behind a hesitation, an unfamiliar acronym, a fear of identity theft, a distrust of the platform's chat, and draft a reply to that worry directly instead of repeating a script, for the host to send. The architecture removes the guest's structural objection. The drafting removes the human friction. A warm, clear, responsive ask through a channel the guest can trust is far more likely to be met than a cold one through a channel they do not. How much more, we intend to measure; either way the answer is useful, because whatever the ask cannot win, the architecture still has to catch.
...
You are reading this inside the open-source vertical reference pack. The architecture is documented; two of the skill files are in. The rest land over the coming weeks. The core loop already runs in our own testing: a guest-registration form filled automatically, field by field, and a host-approved message sent under the host's own account, with each step recorded in the audit chain and the final regulator submission left for the host. The first pilot launches with the Portuguese host who is the subject of much of this essay, this summer.
