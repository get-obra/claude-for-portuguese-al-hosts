...
You are reading this inside the open-source vertical reference pack. The architecture is documented; two of the skill files are in. The rest land over the coming weeks. The core loop already runs in our own testing: a border-authority registration form filled automatically, field by field, and a host-approved message sent under the host's own account, with each step recorded in the audit chain and the final regulator submission left for the host. The first pilot launches with the Portuguese host who is the subject of much of this essay, this summer.
